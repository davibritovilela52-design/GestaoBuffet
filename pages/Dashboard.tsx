...
export default function Dashboard() {
    ...
    return (
        <div className="space-y-10 pb-12">
            ...
            {/* Seção Comercial */}
            <section>
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Performance Comercial</h3>
            ...
            {/* Seção Financeira */}
            <section>
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Indicadores Financeiros</h3>
            ...
            <section>
                ...
                <div className="bg-white dark:bg-[#1a2632] rounded-xl border border-[#dbe0e6] dark:border-gray-800 shadow-sm overflow-hidden">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[#f0f2f4] dark:bg-gray-800/50">
                                ...
                                <tr key={deal.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium">{new Date(deal.eventDate).toLocaleDateString('pt-BR', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
...
function StatCard({ title, value, icon, color, isMoney = false, bgColor = "bg-white dark:bg-[#1a2632]" }: any) {
    return (
        <div className={`${bgColor} p-6 rounded-xl border border-[#dbe0e6] dark:border-gray-800 shadow-sm flex items-center justify-between gap-4 transition-transform hover:-translate-y-1 duration-200`}>
            <div className="flex flex-col gap-1">
                <p className="text-[#617589] dark:text-gray-400 text-xs font-bold uppercase tracking-wider">{title}</p>
                <h3 className="text-2xl md:text-3xl font-black text-[#111418] dark:text-white tracking-tight">
                    {isMoney ? `R$ ${value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : value}
                </h3>
            </div>
            <div className={`w-12 h-12 rounded-full ${color} flex items-center justify-center text-white shadow-lg shadow-gray-200 dark:shadow-none`}>
